...
def write_startfk_model_preview_image(model_path, image):
...
def delete_startfk_model_preview_image(model_path):
    basename, ext = os.path.splitext(model_path)
...
def load_cover_image(model_path, metadata):
    ...
    if len(cover_images) > 0:
        ...
        for preview_path in preview_paths:
            if os.path.isfile(preview_path):
                print(f"[MetadataEditor] Loading panel preview image: {preview_path}")
...
def save_metadata(module, model_path, cover_image, display_name, author, source, keywords, description, rating, tags):
    ...
    if cover_image is None:
        delete_startfk_model_preview_image(model_path)
    else:
        write_startfk_model_preview_image(model_path, cover_image)
